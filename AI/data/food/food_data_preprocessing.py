...
# - 밀도를 고려하여 변환 필요
#     - 수식 : ml x Density(g/ml) = g
# - 영양성분 별로 밀도 설정 필요

# ### 04.Excel을 사용하여 필요 속성을 제외한 속성 제거

# ### 05.데이터셋끼리 merge 및 동일한 식품명이 존재한다면 첫번째 데이터셋 기준으로 하여 나머지 데이터는 제외
# - 첫번째 데이터셋을 기준으로 한 이유 : 1회제공량 속성이 처음부터 존재

# ### 06. Excel을 이용하여 2차 중복제거 수행
# - 동일한 식품이지만 “식품명”의 텍스트가 조금씩 달라 중복 제거가 되지 않을 경우가 존재하기 때문에 수작업으로 제거 

# ### 07.음식 분류 모델의 라벨링 데이터와 음식명 맞추기
