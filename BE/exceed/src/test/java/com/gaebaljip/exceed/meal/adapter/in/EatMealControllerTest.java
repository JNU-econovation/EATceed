...
class EatMealControllerTest extends CommonApiTest {
    ...
    void eatMeal() throws Exception {
        ...
        given(uploadImageUsecase.execute(any()))
                .willReturn(
                        "https://gaebaljip.s3.ap-northeast-2.amazonaws.com/test.jpeb_presignedUrl임");
        ...
        resultActions.andDo(
                document(
                        ...
                        getDocumentResponse(),
                        requestFields(
                                fieldWithPath("multiple")
                                        .type(JsonFieldType.NUMBER)
                                        .description("몇 인분"),
                                fieldWithPath("foodIds")
                                        .type(JsonFieldType.ARRAY)
                                        .description("음식 아이디 리스트"),
                                fieldWithPath("mealType")
                                        .type(JsonFieldType.STRING)
                                        .description("식사 타입" + "BREAKFAST, LUNCH, DINNER, SNACK"),
                                fieldWithPath("fileName")
                                        .type(JsonFieldType.STRING)
                                        .description("파일 이름 + 확장자 있어야함"))));
    }
}
